module loo_pass_data
  ...
  ! C interface
  interface
     subroutine new_loo(indices, keff, albedo, hxyz, flux, &
          total_src_old, total_src, &
...
!===============================================================================
  ...
  subroutine pass_data_into_loo()
    ...
    real (c_double), allocatable, target:: total_src(:,:,:,:)
    ...
    total_src = cmfd % openmc_total_src
    ...
    write(OUTPUT_UNIT, *) flux(1,1,1,1)
    write(OUTPUT_UNIT, *) flux(2,1,1,1)
    write(OUTPUT_UNIT, *) flux(1,2,1,1)
    write(OUTPUT_UNIT, *) flux(2,2,1,1)
    write(OUTPUT_UNIT, *) flux(1,1,2,1)
    write(OUTPUT_UNIT, *) flux(2,1,2,1)
    write(OUTPUT_UNIT, *) flux(1,2,2,1)
    write(OUTPUT_UNIT, *) flux(2,2,2,1)
    call new_loo(c_loc(indices), c_loc(k), c_loc(albedo), &
         c_loc(hxyz), c_loc(flux(1,1,1,1)), &
         c_loc(total_src_old(1,1,1,1)), c_loc(total_src(1,1,1,1)), &
